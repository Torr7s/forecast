...
function getUser() {
  ...
  return client('api/users/me').then((data) => data.user);
}

function login({ email, password }) {
  return client('api/users/auth', { body: { email, password } }).then(
    handleUserResponse
  );
}

function register({ name, email, password }) {
  return client('api/users', { body: { name, email, password } }).then(() =>
